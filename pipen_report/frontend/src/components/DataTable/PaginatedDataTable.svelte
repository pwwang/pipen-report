<script>
    ...
    import { InlineCheckbox } from "carbon-components-svelte/src/Checkbox";
    import {
        Pagination,
        RadioButton,
        Table,
    ...
    $: pagedRows = sorting
        ? paginate(sortedRows, pageSize, page)
...
<TableContainer {...$$restProps}>
    ...
    {/if}
    <slot />
    <Table {zebra} {size} {stickyHeader} {sortable} {useStaticWidth}>
        <TableHead>
            <TableRow>
                {#if expandable}
                    <th
        ...
        <TableBody>
            {#each pagedRows as row, i (row.id)}
                <TableRow
                    id="row-{row.id}"
                ...
                >
                    ...
                    {#each row.cells as cell, j (cell.key)}
                        {#if headers[j].empty}
                            <td
                                class:bx--table-column-menu={headers[j]
                                    .columnMenu}
                            >
    ...
    <Pagination bind:page bind:pageSize totalItems={rows.length} {pageSizes} />
</TableContainer>
